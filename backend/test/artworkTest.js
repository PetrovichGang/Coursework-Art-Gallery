...
describe("Artwork tests", () => {
    ...
    it("It's should add artwork to database", (done) => {
      chai.request(app.server).post("/artwork/create").send(artwork).end((err, res) => {
        assert.equal(res.status, 200)
        artworkId = res.body.id
        chai.request(app.server).get("/artwork/" + artworkId).end((err, res) => {
    ...
    it("It's should update artwork from database", (done) => {
        chai.request(app.server).post("/artwork/update/" + artworkId).send({ sizeX: 256, sizeY: 256 }).end((err, res) => {
          assert.equal(res.status, 200)
          done()
        })
      })
    ...
    it("It's should delete artwork by id from database", (done) => {
        chai.request(app.server).get("/artwork/delete/" + artworkId).end((err, res) => {
          assert.equal(res.status, 200)
          chai.request(app.server).get("/artwork/" + artworkId).end((err, res) => {
            assert.equal(res.status, 200)
            assert.isArray(res.body)
